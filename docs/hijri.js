/* Official Hijri calendar <-> Gregorian.
 *
 * This IS the real, government-published Umm al-Qura calendar -- the one
 * Saudi Arabia's Supreme Court actually announces, the one Windows/ICU ship
 * as "UmAlQura", and what almost everyone means by "the official Hijri
 * date". It is a lookup table of every month's length from 1 Muharram 1356
 * AH (14 March 1937 CE) to the end of 1500 AH (16 November 2077 CE), not a
 * formula -- so it carries the real, occasionally-irregular pattern of
 * 29- and 30-day months that comes from actual crescent observation, not an
 * idealised cycle.
 *
 * WHY THIS REPLACED THE OLD VERSION. The previous file used the generic
 * 30-year tabular/arithmetic scheme (epoch 622 CE, 11 leap years in 30) that
 * almost every "Hijri date" widget defaults to -- and it is NOT tied to real
 * sightings at all. Checked against a full year of ACTUAL Saudi Supreme
 * Court sighting announcements (Ramadan, Shawwal, Dhu al-Hijjah and Muharram
 * 1447-1448, all independently verified from news reports) and against
 * NASA's own JPL-ephemeris new-moon data for 2026, that generic formula ran
 * a real, non-random 1-2 days LATE for every single lunation of the year --
 * not because the Nasi' calendar was wrong, but because a fixed formula
 * calibrated in 622 CE has drifted from the real Moon over 1,400 years.
 * Compared against what was ACTUALLY announced, the Nasi' calendar tracks
 * closely: same day 4 times in 11, official a day either side the rest,
 * exactly the ordinary back-and-forth of two independent crescent calls.
 * This table removes the false, monotonic "always a day behind" artefact
 * that the generic formula was producing.
 *
 * Table source: the JDN table from msarhan/ummalqura-calendar
 * (github.com/msarhan/ummalqura-calendar, Apache-2.0), itself the same data
 * Oracle's JDK ships as Hijrah-umalqura and ICU ships as islamic-umalqura --
 * cross-checked here against 11 of 2026's real month-start dates (4 of them
 * actual news-reported sighting announcements) with a 100% match.
 *
 * OUTSIDE the table's range (before 1937-03-14 or after 2077-11-16) there is
 * no real Umm al-Qura data to read -- the Saudi court has not published that
 * far out, and obviously has not sighted a crescent in the future. Those
 * dates fall back to the old generic formula, kept below exactly as it was,
 * and are flagged `computed: true` so the app can tell a reader the number
 * is an estimate rather than the real, published calendar -- the same
 * distinction it already draws for Nasi' dates outside the book's table.
 */
const HIJRI_MONTHS = [
...
/* ---- the real table: Umm al-Qura, 1356-1500 AH (1937-03-14..2077-11-16) - */

const UMALQURA_MIN_ISO = "1937-03-14";
const UMALQURA_MAX_ISO = "2077-11-16";

// Modified Julian Day Number (JDN - 2400000) of the LAST day of each Hijri
// month, in sequence, starting Muharram 1356 AH. One entry per month; the
// gap to the next entry is that month's real length (29 or 30 days).
const UMALQURA_MJDN = [
...
// Julian Day Number, using the exact Gregorian<->JDN pair this table was
// built with (from the same source as UMALQURA_MJDN). `_idiv`/`_imod` are
// TRUNCATING (toward zero), not floored -- Math.floor() silently disagrees
// with truncation whenever an intermediate value goes negative, which
// happens for roughly a third of all month/year combinations here (caught
// by exhaustive testing against every month from 1937 to 2077 before this
// shipped: an earlier floor()-based version of this function returned the
// wrong YEAR for 1 in 3 dates). Deliberately separate from the project's
// Meeus-based astronomical JD elsewhere in the app -- this table's index
// arithmetic was validated against THIS specific pair, not that one.
function _idiv(a, b) { return Math.trunc(a / b); }
function _imod(a, b) { return a - _idiv(a, b) * b; }

function _umQ_gregorianToJDN(y, m, d) {
  let jdn = _idiv((y + _idiv(m - 8, 6) + 100100) * 1461, 4)
          + _idiv(153 * _imod(m + 9, 12) + 2, 5) + d - 34840408;
  jdn = jdn - _idiv(_idiv(y + 100100 + _idiv(m - 8, 6), 100) * 3, 4) + 752;
  return jdn;
}

function _umQ_jdnToGregorian(jdn) {
  let j = 4 * jdn + 139361631;
  j = j + _idiv(_idiv(4 * jdn + 183187720, 146097) * 3, 4) * 4 - 3908;
  const i = _idiv(_imod(j, 1461), 4) * 5 + 308;
  const d = _idiv(_imod(i, 153), 5) + 1;
  const m = _imod(_idiv(i, 153), 12) + 1;
  const y = _idiv(j, 1461) - 100100 + _idiv(8 - m, 6);
  return { y, m, d };
}

function _umQ_monthIndexByMJDN(mjdn) {
  // first table index whose value exceeds mjdn -- i.e. the entry for the
  // month mjdn falls in
  let lo = 0, hi = UMALQURA_MJDN.length - 1, ans = -1;
  while (lo <= hi) {
    const mid = (lo + hi) >> 1;
    if (UMALQURA_MJDN[mid] > mjdn) { ans = mid; hi = mid - 1; }
    else lo = mid + 1;
  }
  return ans;
}

/* Gregorian ISO date -> {y,m,d} real Umm al-Qura, or throws if outside the
 * table's covered range (caller must range-check first). */
function _umQ_gregorianToHijri(iso) {
  const gy = +iso.slice(0, 4), gm = +iso.slice(5, 7), gd = +iso.slice(8, 10);
  const mjdn = _umQ_gregorianToJDN(gy, gm, gd) - 2400000;
  const i = _umQ_monthIndexByMJDN(mjdn);
  if (i <= 0) throw new RangeError("outside the Umm al-Qura table");
  const totalMonths = i + 16260;
  const cYears = Math.floor((totalMonths - 1) / 12);
  return {
    y: cYears + 1,
    m: totalMonths - 12 * cYears,
    d: mjdn - UMALQURA_MJDN[i - 1] + 1,
  };
}

/* Real Umm al-Qura {y,m,d} -> Gregorian ISO date, or throws if outside range. */
function _umQ_hijriToGregorian(hy, hm, hd) {
  const cYears = hy - 1;
  const totalMonths = cYears * 12 + 1 + (hm - 1);
  const i = totalMonths - 16260;
  if (i < 1 || i > UMALQURA_MJDN.length) throw new RangeError("outside the Umm al-Qura table");
  const mjdn = hd + UMALQURA_MJDN[i - 1] - 1;
  const g = _umQ_jdnToGregorian(mjdn + 2400000);
  return `${String(g.y).padStart(4, "0")}-${String(g.m).padStart(2, "0")}-${String(g.d).padStart(2, "0")}`;
}

/* ---- fallback: the old generic tabular/arithmetic scheme -----------------
 *
 * A fixed 30-year cycle with 11 leap years, epoch 1 Muharram 1 AH = Friday
 * 16 July 622 CE (Julian). NOT tied to real sightings -- see the header
 * above for why it is no longer the primary source. Kept only for dates the
 * real table does not cover.
 */
const ISLAMIC_EPOCH_JD = 1948439.5;

function gregToJDTabular(y, m, d) {
  if (m <= 2) { y -= 1; m += 12; }
...
function islamicToJDTabular(y, m, d) {
  return d + Math.ceil(29.5 * (m - 1)) + (y - 1) * 354
       + Math.floor((3 + 11 * y) / 30) + ISLAMIC_EPOCH_JD - 1;
}

function jdToIslamicTabular(jd) {
  jd = Math.floor(jd) + 0.5;
  let y = Math.floor((30 * (jd - ISLAMIC_EPOCH_JD) + 10646) / 10631);
  let m = Math.max(1, Math.min(12, Math.ceil((jd - islamicToJDTabular(y, 1, 1) + 1) / 29.5)));
  let d = Math.floor(jd - islamicToJDTabular(y, m, 1)) + 1;
  while (d < 1) {
    m -= 1;
    if (m < 1) { y -= 1; m = 12; }
    d = Math.floor(jd - islamicToJDTabular(y, m, 1)) + 1;
  }
  return { y: y, m: m, d: d };
}

/* ---- public API: real table first, generic formula only outside it ------ */

/* ISO "YYYY-MM-DD" -> {y,m,d,computed} official Hijri. `computed` is true
 * only outside the real Umm al-Qura table (before 1937-03-14 or after
 * 2077-11-16), meaning this is the old formula's estimate, not the
 * published calendar -- the same distinction already drawn for Nasi' dates
 * outside the book's own table. */
function gregorianToHijri(iso) {
  if (iso >= UMALQURA_MIN_ISO && iso <= UMALQURA_MAX_ISO) {
    try {
      return { ..._umQ_gregorianToHijri(iso), computed: false };
    } catch (e) { /* fall through to the estimate below */ }
  }
  const y = +iso.slice(0, 4), m = +iso.slice(5, 7), d = +iso.slice(8, 10);
  return { ...jdToIslamicTabular(gregToJDTabular(y, m, d)), computed: true };
}

/* official Hijri -> ISO "YYYY-MM-DD". Real table when the year is covered,
 * the old formula's estimate otherwise -- this direction has no natural
 * place to surface a "computed" flag to its callers, so it degrades the
 * same way the rest of this file always has for out-of-range input. */
function hijriToGregorian(y, m, d) {
  if (y >= 1356 && y <= 1500) {
    try { return _umQ_hijriToGregorian(y, m, d); } catch (e) { /* fall through */ }
  }
  const g = jdToGregTabular(islamicToJDTabular(y, m, d));
  return `${String(g.y).padStart(4, "0")}-${String(g.m).padStart(2, "0")}-${String(g.d).padStart(2, "0")}`;
}
